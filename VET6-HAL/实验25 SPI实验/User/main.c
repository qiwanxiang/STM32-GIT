...
int main(void)
{
    ...
    flashsize = 8 * 1024 * 1024;   /* FLASH 大小为16M字节 */
    ...
    while (1)
    {
        ...
        if (key == KEY1_PRES)   /* KEY1按下,写入 */
        {
            ...
            norflash_write((uint8_t *)datatemp, flashsize - 160, sizeof(datatemp));      /* 从倒数第100个地址处开始,写入SIZE长度的数据 */
        ...
        if (key == KEY0_PRES)   /* KEY0按下,读取字符串并显示 */
        {
            ...
            norflash_read(datatemp, flashsize - 160, sizeof(datatemp));                   /* 从倒数第100个地址处开始,读出SIZE个字节 */
