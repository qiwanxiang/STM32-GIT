...
void load_draw_dialog(void)
{
    lcd_clear(WHITE);                                                /* 清屏 */
    g_back_color=RED;
    lcd_show_string(lcddev.width - 36, 0, 200, 24, 24, "RST", BLUE); /* 显示清屏区域 */
    g_back_color=WHITE;      /* 背景颜色.默认为白色 */
}
...
void rtp_test(void)
{
    ...
    while (1)
    {
        ...
        if (tp_dev.sta & TP_PRES_DOWN)  /* 触摸屏被按下 */
        {
            ...
            {
                if (tp_dev.x[0] > (lcddev.width - 36) && tp_dev.y[0] < 24)
                {
                    load_draw_dialog();
                }
                else
