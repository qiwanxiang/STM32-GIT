...
)
{
    ...
    if (pdrv == SD_CARD)    /* SD卡 */
    {
        switch (cmd)
        {
            ...
            case GET_BLOCK_SIZE:
                *(WORD *)buff = g_sdcard_handle.SdCard.BlockSize;//g_sd_card_info.CardBlockSize;
                res = RES_OK;
                break;

            case GET_SECTOR_COUNT:
                *(DWORD *)buff = ((long long)g_sdcard_handle.SdCard.BlockNbr*g_sdcard_handle.SdCard.BlockSize)/512;//g_sd_card_info.CardCapacity / 512;
                res = RES_OK;
                break;
