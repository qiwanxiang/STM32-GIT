...
static void lv_event_bit_map_handler(lv_event_t *event)
{
    ...
    if(code == LV_EVENT_CLICKED)
    {
        ...
        if (lv_btnmatrix != NULL)
        {
            lv_obj_del(lv_btnmatrix); /* 删除键盘 */
            lv_textarea_set_cursor_click_pos(lv_middle, true);  /* 其他文本区域有效隐藏 */
            lv_middle = NULL;
            lv_btnmatrix = NULL;
        }
        ...
        const char * lv_show_lv_atk_str_buffer = lv_textarea_get_placeholder_text(lv_bin);                    /* 获取输入内容 */
        ...
        lv_textarea_set_placeholder_text(lv_oct, lv_oct_buf);                                                 /* 十进制转八进制 */
...
static void lv_btnmatrix_event_handler(lv_event_t *event)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED)
    {
        ...
        for (int i = 0 ;i < 20 ;i ++)
        {
            if (txt == btnm1_map[i])
            {
                lv_textarea_add_text(lv_middle,(const char *)txt);
                break;
            }
        }
...
static void lv_ta_cb_event_handler(lv_event_t *event)
{
    ...
    if((code == LV_EVENT_CLICKED) && (lv_btnmatrix == NULL))
    {
        lv_middle = NULL;
        lv_textarea_set_cursor_click_pos(ta, false);               /* 失能游标隐藏 */
        lv_btnmatrix = lv_btnmatrix_create(lv_scr_act()); /* 创建矩阵按键 */
        lv_btnmatrix_set_map(lv_btnmatrix, btnm1_map);          /* 设置矩阵按键数组 */
        lv_btnmatrix_set_btn_width(lv_btnmatrix, 12, 1);        /* 设置按键的比例 */
        lv_obj_set_size(lv_btnmatrix,lcddev.width,lcddev.height/2);    /* 设置矩阵按键的大小 */
        lv_obj_align_to(lv_btnmatrix, NULL, LV_ALIGN_BOTTOM_MID, 0, 0);/* 设置对齐模式 */
        lv_obj_add_event_cb(lv_btnmatrix, lv_btnmatrix_event_handler,LV_EVENT_ALL,NULL); /* 设置矩阵按键的回调函数 */
        ...
        if (ta == lv_bin) /* 按下是二进制的文本区域实现代码 */
        {
            for (int i = 0 ; i < 18 ; i++)
            {
              if (i == 0 || i == 9 || i == 16 || i == 17) /* 矩阵按键的索引号0，9，16，17不做处理 */
                ...
                {
                    lv_btnmatrix_set_btn_ctrl(lv_btnmatrix, i, LV_BTNMATRIX_CTRL_DISABLED); /* 失能这些索引 */
                }
            }
            lv_middle = lv_bin; /* 中间对象等于二进制文本区域对象 */
            lv_textarea_set_cursor_click_pos(lv_dec, true);  /* 其他文本区域有效隐藏 */
        ...
        else if (ta == lv_dec)
        {
            ...
            lv_middle = lv_dec;
            lv_textarea_set_cursor_click_pos(lv_bin, true);  /* 其他文本区域有效隐藏 */
            lv_textarea_set_cursor_click_pos(lv_hex, true);
            lv_textarea_set_cursor_click_pos(lv_oct, true);
        }
        else if (ta == lv_hex)
    ...
    else if(code == LV_EVENT_VALUE_CHANGED) /* 如果文本区域的数值发生改变，就执行下面的代码 */
    {
        ...
        else if (ta == lv_dec) /* 如果是十进制文本区域的数值改变 */
        {
            if (lv_atk_str_buffer != NULL)
            {
                /*  十进制转二进制 */
        ...
        else if (ta == lv_hex) /* 如果是十六进制文本区域的数值改变 */
        {
            if (lv_atk_str_buffer != NULL)
            {
                memset(lv_oct_buf,0,sizeof(lv_oct_buf));
                ...
                lv_textarea_set_placeholder_text(lv_oct, lv_dex_oct_buf);                                                      /* 十进制转八进制 */
                lv_snprintf(str_tmp, sizeof(str_tmp),"0x%X", lv_atk_bit_bin_to_dec(regbit_flag_buf));                          /* 二进制转十六进制 */
                lv_textarea_set_placeholder_text(lv_hex, str_tmp);
            }
            else
        ...
        else if (ta == lv_oct) /* 如果是八进制文本区域的数值改变 */
        {
            if (lv_atk_str_buffer != NULL)
            {
                memset(lv_oct_buf,0,sizeof(lv_oct_buf));
...
void lv_atk_scale_bit_init(lv_obj_t *parent)
{
    ...
    {
        /*  位按钮编号索引 */
        lv_bit_index[i] = lv_obj_create(parent);    /*  创建obj对象 */
        lv_obj_set_style_bg_color(lv_bit_index[i], lv_color_hex3(0x00), LV_STATE_DEFAULT);    /*  设置颜色 */
        lv_obj_set_style_border_side(lv_bit_index[i], 0, LV_STATE_DEFAULT);    /*  设置边界宽度 */
        lv_obj_set_style_text_color(lv_bit_index[i], lv_color_hex3(0xffffff), LV_STATE_DEFAULT);  /*  设置字符 */
        lv_obj_set_style_radius(lv_bit_index[i], 2, LV_STATE_DEFAULT);         /*  设置圆角 */
        lv_obj_set_size(lv_bit_index[i], LV_ATK_BIT_W, LV_ATK_BIT_INDEX_H); /*  设置对象大小(宽度和高度) */ 

        /*  位按钮 */
        lv_bit_text[i] = lv_btn_create(parent);    /*  创建按钮对象 */
        lv_obj_set_style_radius(lv_bit_text[i], 5, LV_STATE_DEFAULT); /*  设置圆角 */
        lv_obj_set_style_bg_color(lv_bit_text[i], lv_palette_main(LV_PALETTE_RED), LV_STATE_DEFAULT); /*  设置颜色 */
        lv_obj_set_style_border_side(lv_bit_text[i], 0, LV_STATE_DEFAULT);  /*  设置边界宽度*/ 
        lv_obj_set_style_text_color(lv_bit_text[i], lv_color_hex3(0xffffff), LV_STATE_DEFAULT);  /*  设置字符 */
        lv_obj_set_size(lv_bit_text[i], LV_ATK_BIT_W, LV_ATK_BIT_H);  /*  设置对象大小(宽度和高度) */
        lv_obj_add_event_cb(lv_bit_text[i], lv_event_bit_map_handler,LV_EVENT_ALL,NULL);  /*  分配点击事件处理函数 */
        /* 位按钮数字 */
        lv_obj_text  = lv_label_create(lv_bit_text[i]);  /* 创建标签 */
        lv_label_set_text(lv_obj_text, "0");
        lv_obj_align_to(lv_obj_text, NULL, LV_ALIGN_CENTER, 0, 0);  /* 当前画布居中 */
        lv_obj_align_to(lv_bit_index[i], parent, LV_ALIGN_TOP_RIGHT, -(LV_ATK_BIT_ROW_SPACE*(lv_scale_col_1 ++ )+3), 50); /*  设置摆放位置 */
        lv_obj_align_to(lv_bit_text[i], lv_bit_index[i], LV_ALIGN_OUT_BOTTOM_MID, 0, 0);  /*  设置摆放位置 */
        
        if (i == 32)
        {
            lv_obj_set_style_bg_color(lv_bit_text[32], lv_palette_main(LV_PALETTE_BLUE), LV_STATE_DEFAULT); /*  设置颜色 */
            lv_obj_set_style_bg_color(lv_bit_index[32], lv_palette_main(LV_PALETTE_BLUE), LV_STATE_DEFAULT); /*  设置颜色 */
            lv_label_set_text(lv_obj_get_child(lv_bit_text[32],NULL), "");
        }
    ...
    lv_obj_align_to(lv_bin_label,lv_bit_text[LV_ATK_BIT_COUNT - 1],LV_ALIGN_OUT_BOTTOM_MID,0,20);
    /*  二进制 */
    lv_bin = lv_textarea_create(parent);
    lv_textarea_set_accepted_chars(lv_bin, "01");  /*  设置字符列表，文本区域只接受这些字符 */
    lv_obj_set_style_text_font(lv_bin, &lv_font_montserrat_14, LV_STATE_DEFAULT);  /*  设置数值字体格式 */
    lv_obj_set_style_radius(lv_bin, 0, LV_STATE_DEFAULT);  /*  设置圆角 */
    lv_obj_set_size(lv_bin,lcddev.width/2,LV_ATK_BIT_H);
    lv_textarea_set_text(lv_bin, "");
    ...
    lv_obj_align_to(lv_bin, lv_bin_label, LV_ALIGN_OUT_RIGHT_MID, 12, 0);  /*  摆放位置 */
    
    
    lv_dec_label = lv_label_create(parent);
    lv_label_set_text(lv_dec_label,"DEC");
    lv_obj_align_to(lv_dec_label,lv_bin_label,LV_ALIGN_OUT_BOTTOM_MID,0,20);
    /*  十进制 */
    lv_dec = lv_textarea_create(parent);
    lv_textarea_set_accepted_chars(lv_dec, "0123456789");  /*  设置字符列表，文本区域只接受这些字符 */
    lv_obj_set_style_text_font(lv_dec, &lv_font_montserrat_14, LV_STATE_DEFAULT);  /*  设置文本字体格式 */
    lv_obj_set_style_radius(lv_dec, 0, LV_STATE_DEFAULT);  /*  设置圆角 */
    lv_obj_set_size(lv_dec,lcddev.width/2,LV_ATK_BIT_H);
    lv_textarea_set_text(lv_dec, "");
    lv_textarea_set_one_line(lv_dec, lv_dec);
    lv_textarea_set_cursor_click_pos(lv_dec, true);
    lv_obj_add_event_cb(lv_dec, lv_ta_cb_event_handler,LV_EVENT_ALL,NULL);
    lv_obj_align_to(lv_dec, lv_dec_label, LV_ALIGN_OUT_RIGHT_MID, 10, 0);  /*  摆放位置 */
    
    lv_hex_label = lv_label_create(parent);
    lv_label_set_text(lv_hex_label,"HEX");
    lv_obj_align_to(lv_hex_label,lv_dec_label,LV_ALIGN_OUT_BOTTOM_MID,0,20);
    /*  十六进制 */
    lv_hex = lv_textarea_create(parent);
    lv_textarea_set_accepted_chars(lv_hex, "0123456789ABCDEFabcdef");  /*  设置字符列表，文本区域只接受这些字符 */
    lv_obj_set_style_text_font(lv_hex, &lv_font_montserrat_14, LV_STATE_DEFAULT);  /*  设置文本字体格式 */
    lv_obj_set_style_radius(lv_hex, 0, LV_STATE_DEFAULT);        /*  设置圆角 */
    lv_obj_set_size(lv_hex,lcddev.width/2,LV_ATK_BIT_H);
    lv_textarea_set_text(lv_hex, "");
    lv_textarea_set_one_line(lv_hex, true);
    lv_textarea_set_cursor_click_pos(lv_hex, true);
    lv_obj_add_event_cb(lv_hex, lv_ta_cb_event_handler,LV_EVENT_ALL,NULL);
    lv_obj_align_to(lv_hex, lv_hex_label, LV_ALIGN_OUT_RIGHT_MID, 10, 0);  /*  摆放位置 */
    
    lv_oct_label = lv_label_create(parent);
    lv_label_set_text(lv_oct_label,"OCT");
    lv_obj_align_to(lv_oct_label,lv_hex_label,LV_ALIGN_OUT_BOTTOM_MID,0,20);
    /*  八进制 */
    lv_oct = lv_textarea_create(parent);
    lv_textarea_set_accepted_chars(lv_oct, "01234567");  /*  设置字符列表，文本区域只接受这些字符 */
    lv_obj_set_style_text_font(lv_oct, &lv_font_montserrat_14, LV_STATE_DEFAULT);  /*  设置文本字体格式 */
    lv_obj_set_style_radius(lv_oct, 0, LV_STATE_DEFAULT);        /*  设置圆角 */
    lv_obj_set_size(lv_oct,lcddev.width/2,LV_ATK_BIT_H);
    lv_textarea_set_text(lv_oct, "");
    lv_textarea_set_one_line(lv_oct, true);
    lv_textarea_set_cursor_click_pos(lv_oct, true);
    lv_obj_add_event_cb(lv_oct, lv_ta_cb_event_handler,LV_EVENT_ALL,NULL);
    lv_obj_align_to(lv_oct, lv_oct_label, LV_ALIGN_OUT_RIGHT_MID, 10, 0);  /*  摆放位置 */
}
...
void lv_mainstart(void)
{
    ...
    lv_scale_cont = lv_obj_create(lv_scr_act());                    /* 创建一个容器 */
    lv_obj_set_size(lv_scale_cont, lcddev.width, lcddev.height);    /* 设置容器的大小 */
    lv_obj_set_pos(lv_scale_cont,0,0);                              /* 设置容器的位置 */
    lv_btnmatrix = NULL;                                            /* 设置键盘对象位空 */
    lv_atk_scale_bit_init(lv_scale_cont);                           /*  初始化界面 */
}
